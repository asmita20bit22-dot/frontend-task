...
function ProfileCard({ image, age, name, role, email, status, objData }) {
  const isActive = status.toLowerCase() === "active";
  ...
  return (
    <div className="group bg-white rounded-2xl shadow-md hover:shadow-2xl border border-gray-100 p-6 flex flex-col items-center text-center transition-all duration-300 hover:-translate-y-1">
      ...
      <div className="relative mb-4">
        ...
        <img src={image} alt={name} className="relative w-24 h-24 rounded-full object-cover p-1" />
      </div>
      ...
      {age && <p className="text-sm text-gray-400 mb-4">Age: {age}</p>}

      {objData?.name && (
        <p className="text-sm text-gray-400 mb-4 break-all">{objData.name}</p>
      )}
